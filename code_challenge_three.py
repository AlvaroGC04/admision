...
a better one.
Hint 2
Start by sorting the input array. Since you’re trying to find the minimum amount of change that
you can’t create, it makes sense to consider the smallest coins first.
Hint 3
To understand the trick to this problem, consider the following example: coins = [1, 2, 4]. With this
set of coins, we can create 1, 2, 3, 4, 5, 6, 7 cents worth of change. Now, if we were to add a coin
of value 9 to this set, we would not be able to create 8 cents. However, if we were to add a coin of
...
"""
def minimum_change(money):
    money.sort()
    change = 1
    for coin in money:
        if coin > change: #Si esto sucede es porque no existe ninguna combinación que de este dinero y frenaria debido a que esta ordenado
            break
        change = change + coin #La moneda que sigue no puede ser mayor que la suma que llevo.
    print(f"El cambio mínimo que no puedo dar es: {change}")
